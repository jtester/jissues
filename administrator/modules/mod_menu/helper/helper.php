...
//		$query->select('m.id, m.title, m.alias, m.link, m.parent_id, m.img, e.element');
		$query->select('e.name AS title, e.element, e.extension_id AS id');
...
//		$query->order('m.lft');
		...
		$blacklist = array('com_login', 'com_cpanel');
		$blacklist[] = 'com_languages';
		$blacklist[] = 'com_users';

		// Parse the list of extensions.
		foreach ($components as &$component) {
			if(in_array($component->element, $blacklist))
			{
				continue;
			}

			// Trim the menu link.
			$component->link = trim('index.php?option='.$component->element);
			$component->img = 'edit';

			if (1){//$component->parent_id == 1) {
				...
				if ($authCheck == false || ($authCheck && $user->authorise('core.manage', $component->element))) {
					...
					$component->text = $lang->hasKey($component->title) ? JText::_($component->title) : $component->title;
				}
			} else {
				// Sub-menu level.
